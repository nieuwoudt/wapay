 ...
 */
/**
 * How OTT stringifies its own typed fields before hashing. Their echo of a
 * parsed request (sandbox, 2026-09-23) shows `amount` as a number (20, not
 * "20.00"), `providerCode` as an int and `bank_id` defaulting to 0, so the
 * hash they compute may differ from ours by exactly those renderings.
 * `hashStyle` lets the probe try each rendering; the default is what the
 * sandbox proved on 2026-09-23 (see docs/OTT_PAYOUT_API.md §6).
 *   amount: '2dp' → "20.00"   | 'plain' → "20" / "20.5" / "20.05" (double.ToString)
 *   bankId: 'empty' → ""      | 'zero' → "0" when absent
 */
// PROVEN on OTT's sandbox 2026-09-23 (probe, Nedbank cardless R20 → status 100,
// paymentReference 126141): amount "20.00" as documented, and an absent bank_id
// hashed as "0" (their Int32 default), NOT "". "plain" amounts fail with status 2.
export const DEFAULT_HASH_STYLE = Object.freeze({ amount: '2dp', bankId: 'zero' });
export function plainAmountString(amountCents) {
  return String(Number(centsToAmountString(amountCents)));
}
function performPayoutHashValues({ amount, amountCents, provider, recipient, yourUniqueReference, hashStyle = DEFAULT_HASH_STYLE }) {
  const r = recipient || {};
  const p = provider || {};
  const hashAmount = hashStyle?.amount === 'plain' && Number.isInteger(amountCents) ? plainAmountString(amountCents) : amount;
  const hashBankId = r.bank_id !== undefined && r.bank_id !== null && r.bank_id !== '' ? r.bank_id : hashStyle?.bankId === 'zero' ? '0' : r.bank_id;
  return [
    r.account_name,
    r.account_number,
    hashAmount,
...
/**
 * OTT's payout API is a .NET service. A JSON field typed as an integer on
 * their side (bank_id) rejects "" with HTTP 400 "The JSON value could not be
 * converted to System.Int32" before the request is processed, and every
 * absent optional we used to send as "" tripped it: the founder's pay-outs of
 * 2026-09-15 (PayShap) and 2026-09-17 (Nedbank) never existed at OTT
 * (BUGLOG #80). On the wire: omit empty fields, send the integer-typed ones
 * as numbers. The HASH is unchanged: it is computed over the string values
 * with absent optionals as "" (spec), independent of JSON typing.
 */
const WIRE_INT_FIELDS = new Set(['bank_id']);
export function wireRecipient(recipient = {}) {
  const out = {};
  for (const [k, v] of Object.entries(recipient)) {
    if (v === undefined || v === null || v === '') continue;
    out[k] = WIRE_INT_FIELDS.has(k) && /^\d+$/.test(String(v)) ? Number(v) : v;
  }
  return out;
}

/**
 * ASP.NET answers a request its model binder could not read with HTTP 4xx
 * and an RFC 7807 problem document ({ type, title, status, errors }). Its
 * `status` is the HTTP code, never an OTT payout status (400 fed into the
 * status table read as UNKNOWN and parked the pay-out), and no payout exists.
 */
export function isProblemDetails(httpStatus, body) {
  return (
    Number(httpStatus) >= 400 &&
    !!body && typeof body === 'object' && !Array.isArray(body) &&
    ('errors' in body || 'title' in body || 'type' in body) &&
    !('paymentReference' in body)
  );
}
/** The field paths a problem document names (never the values). */
export function problemPaths(body) {
...
export class OttPayoutClient {
   ...
   */
  async performPayout({ amountCents, providerCode, providerName = '', recipient, yourUniqueReference, optionalData, hashStyle = DEFAULT_HASH_STYLE }) {
    ...
    const body = {
      ...
      amount,
      provider,
      // Empty optionals are NOT sent (BUGLOG #80); the hash below still sees them as "".
      recipient: wireRecipient(recipient),
      ...(optionalData ? { optionalData } : {}),
    };
    const hashValues = performPayoutHashValues({ amount, amountCents, provider, recipient, yourUniqueReference, hashStyle });
    // The exact request, for an operator answering "share your request" (the probe route masks it).
    const wire = { url: `${this.base}/api/purchase/v1/PerformPayout`, encoding: this.bodyEncoding, hashStyle, body: { ...body, hashcheck: payoutHash(hashValues, this.apiKey) } };
    ...
    if (isProblemDetails(httpStatus, respBody)) {
      // The framework refused the request before OTT processed it. A 400 that
      // names fields is a validation failure: nothing was paid, release. Any
      // other problem document is unexplained: keep the hold and reconcile.
      const paths = problemPaths(respBody);
      const validation = Number(httpStatus) === 400 && paths.length > 0;
      log('ott_payout_request_rejected', { ref: yourUniqueReference, providerCode, amountCents, httpStatus, title: String(respBody.title || '').slice(0, 80), paths });
      return validation
        ? { httpStatus, status: null, outcome: 'REQUEST_INVALID', settlement: 'RELEASE', retriable: false, paymentReference: null, body: respBody, errors: paths, wire }
        : { httpStatus, status: null, outcome: `HTTP_${httpStatus}`, settlement: 'PENDING', retriable: false, reconcileRequired: true, paymentReference: null, body: respBody, errors: paths, wire };
    }
    ...
    return { httpStatus, status, ...klass, paymentReference: respBody?.paymentReference || null, body: respBody, wire };
  }
