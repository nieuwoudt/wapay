...
function computeExternalCode(p, vendorId) {
  ...
  const raw = JSON.stringify({
    ...
    vendorId,
    name: p?.name || '',
    amountCents: p?.amountCents ?? p?.priceCents ?? 0,
...
function buildUnitType(category) {
  if (category === 'AIRTIME') return 'CURRENCY';
  // Map SMS and everything else to DATA for now
  return 'DATA';
}

function buildVasProductRecord({ p, normalized, canonicalCategory, networkCode, externalCode, fixedPriceCents, dataMb, validityDays, periodType }) {
  let unitType = normalized.unitType || normalized.metadata?.unitType || null;
  if (!unitType) {
    if (canonicalCategory === 'AIRTIME') {
      unitType = 'CURRENCY';
    } else if (canonicalCategory === 'SMS') {
      unitType = 'DATA';
    } else {
      unitType = 'DATA';
    }
  ...
  if (!unitType) {
    unitType = buildUnitType(canonicalCategory);
  }
...
export async function syncBluDataCatalogue({ vendors = ['vodacom', 'mtn', 'cellc', 'telkom'] } = {}) {
  ...
  let airtimeCurrencyCount = 0;
  let dataDataCount = 0;
  let smsMappedToDataCount = 0;
  let missingUnitTypeDefaultedCount = 0;
  ...
  for (const vendorId of vendors) {
    ...
    try {
      ...
      console.error(JSON.stringify({
        type: 'catalog_sync_vendor_error',
        vendorId,
        networkCode,
        error,
        errorDetail,
    ...
    if (error) {
      results.push({ vendorId, networkCode, ok: false, error, errorDetail, count: 0, ms: Date.now() - startedAt });
      continue;
    }
    ...
    for (const p of filtered) {
      const externalCode = computeExternalCode(p, p.vendorId || vendorId);
      ...
      const record = buildVasProductRecord({
        p,
        normalized,
        canonicalCategory,
        networkCode,
      ...
      if (record.unitType === 'CURRENCY') airtimeCurrencyCount++;
      else if (canonicalCategory === 'SMS') smsMappedToDataCount++;
      else dataDataCount++;
      ...
      try {
        await prisma.vasProduct.upsert({
          ...
          update: {
            ...record,
            // keep purchaseType/targetType stable even on update
        ...
        console.error(JSON.stringify({
          type: 'catalog_sync_upsert_error',
          provider: 'BLU',
          category: canonicalCategory,
          externalCode,
  ...
  console.log(JSON.stringify({
    ...
    unitTypeDefaults: {
      airtimeCurrencyCount,
      dataDataCount,
      smsMappedToDataCount,
      missingUnitTypeDefaultedCount,
    },
    timestamp: new Date().toISOString(),
  }));
