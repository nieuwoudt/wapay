...
/**
 * Give a claimed WhatsApp message id back so Meta's redelivery can process it.
 * Only for a turn that threw BEFORE sending anything to the customer; the
 * webhook checks that with the outbound send counter. Never throws: a failed
 * release simply leaves the claim in place (the message is then dropped, the
 * pre-existing behaviour).
 */
export async function releaseClaim({ waMessageId }) {
  if (!waMessageId) return { released: false };
  try {
    const { count } = await prisma.processedMessage.deleteMany({ where: { waMessageId } });
    return { released: count > 0 };
  } catch (err) {
    console.error(JSON.stringify({ type: 'wa_claim_release_error', waMessageId, error: err?.message }));
