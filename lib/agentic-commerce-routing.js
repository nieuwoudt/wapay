...
export function decideCommerceRoute({ text = '', currentState = null, stateData = null }) {
  ...
  if (slots.productHint === 'ELECTRICITY') {
    if (slots.amountCents && slots.meterNumber) {
      return { routeDecision: 'ELECTRICITY_CONFIRM', nextState: 'ELECTRICITY_CONFIRM', missing: [], slots };
    }
    if (slots.amountCents && !slots.meterNumber) {
      return { routeDecision: 'ELECTRICITY_METER', nextState: 'ELECTRICITY_METER', missing: ['meterNumber'], slots };
    }
    if (!slots.amountCents && slots.meterNumber) {
      return { routeDecision: 'ELECTRICITY_AMOUNT', nextState: 'ELECTRICITY_AMOUNT', missing: ['amountCents'], slots };
    }
  }
