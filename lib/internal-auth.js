...
export function requireInternalAuth(req, res) {
  ...
  if (!expected) {
    // 2026-09-18: fail CLOSED in production. The fail-open branch below was
    // written when the variable was not configured anywhere; it now is, and
    // all five execute routes answer 401 to an unauthenticated call. Leaving
    // it open meant one deleted environment variable would silently reopen
    // every money route, which is the worst kind of outage: invisible.
    // Outside production it still passes, so a local run needs no key.
    if (process.env.NODE_ENV === 'production') {
      logStructured('internal_auth_misconfigured', { path: req?.url });
      res.status(503).json({ error: 'CONFIG', message: 'Service unavailable' });
      return false;
    }
    if (!warnedUnenforced) {
