...
/**
 * A money outcome the customer was NOT told about, written into the same
 * history the agent reads (role 'event', rendered to the model as "[event] …").
 *
 * The hole this closes: when a pay-out settles at 02:00 and every rail fails,
 * the ledger has moved and the customer's next turn begins with the agent
 * knowing nothing about it, so it answers "did my withdrawal go through" from a
 * record whose movement list changed for reasons it cannot explain. An event
 * row is the ledger fact; an assistant turn is what we said. They are different
 * and only the second one existed.
 *
 * Deliberately written ONLY when nothing was delivered, so a successful
 * notification does not appear in history twice.
 *
 * Never throws.
 * @returns {Promise<{ ok: boolean }>}
 */
export async function recordMoneyEvent({ prisma: db = prisma, accountId, text, kind = 'reconcile', refs = null } = {}) {
  try {
    if (!accountId || !text) return { ok: false };
    const turns = await import('./turns.js');
    const r = await turns.recordTurn({ prisma: db, accountId, role: 'event', text, kind, refs });
    log('money_event_recorded', { accountId, kind, ok: !!r?.ok });
    return { ok: !!r?.ok };
  } catch (error) {
    log('money_event_failed', { accountId, kind, error: error?.message });
    return { ok: false };
  }
}
