 ...
 * spend online where OTT vouchers are accepted — never a cash transfer.
 *
...
export function resolveGift({ slots = {}, senderMsisdn, product } = {}) {
  ...
  // and the recipient can spend it online where OTT vouchers are accepted.
  if (prod === 'SEND_MONEY') {
 ...
 */
/**
 * Claim message for a wiCode (Yoyo rail) voucher — fuel/retail at physical
 * tills, not the OTT online network. Same bearer rules as the OTT claim:
 * verbatim English, never localized, never logged.
 *
 * @param {object} args
 * @param {number} args.amountCents
 * @param {string} args.wicode - the bearer code (delivered, never logged)
 * @param {string} [args.guide] - redemption guide copy (lib/spend-catalogue.js)
 */
export function buildWicodeClaimMessage({ amountCents, wicode, guide } = {}) {
  const lines = [
    `⛽ Your ${rands(amountCents)} UniFuel fuel voucher is ready!`,
    '',
    `Your UniFuel voucher code: ${wicode}`,
  ];
