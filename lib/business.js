...
import { businessSignupAllowlist } from './business-auth.js';
...
import { paymentRequestFeeCents, centsToRandString, PAYREQ_FREE_BELOW_CENTS, vatRegistered } from './deposits.js';
import { primaryCardRail } from './adumo.js';
...
export function quoteLink(amountCents) {
  const rail = primaryCardRail();
  const feeCents = paymentRequestFeeCents(amountCents, rail);
  return { amountCents, feeCents, netCardCents: amountCents - feeCents, netBalanceCents: amountCents, freeBelowCents: PAYREQ_FREE_BELOW_CENTS, rail, feeIncludesVat: vatRegistered() };
}
...
/**
 * Attach a customer to a link created as a walk-in (founder 2026-09-10: "enter
 * the customer number here and send"). Creates or reuses the customer row,
 * files the OPEN link under it (only while it has no customer: an atomic
 * conditional update, so two tabs cannot re-home a ticket), and returns the
 * link with the ready message and the owner's WhatsApp deep link.
 */
export async function attachCustomerToLink({ prisma: prismaClient = prisma, business, code, msisdn, name }) {
  const request = await prismaClient.paymentRequest.findUnique({ where: { id: String(code || '').toUpperCase() } });
  if (!request || request.businessId !== business.id) throw Object.assign(new Error('Unknown link'), { code: 'NOT_FOUND' });
  if (request.status !== 'PENDING' || request.expiresAt <= new Date()) throw Object.assign(new Error('Link is not open'), { code: 'NOT_OPEN' });
  const { customer } = await upsertCustomer({ prisma: prismaClient, businessId: business.id, msisdn, name, source: 'MANUAL' });
  if (!request.customerId) {
    await prismaClient.paymentRequest.updateMany({ where: { id: request.id, businessId: business.id, customerId: null }, data: { customerId: customer.id } });
  } else if (request.customerId !== customer.id) {
    throw Object.assign(new Error('Link already belongs to another customer'), { code: 'ALREADY_ATTACHED' });
  }
  const fresh = await prismaClient.paymentRequest.findUnique({ where: { id: request.id } });
  const items = Array.isArray(fresh.items) ? fresh.items : [];
  const url = paymentRequestUrl(fresh.id);
  const message = composeLinkMessage({ businessName: business.name, customerName: customer.name, amountCents: fresh.amountCents, items, reference: fresh.reference, note: fresh.note, url });
  log('business_link_attached', { businessId: business.id, code: fresh.id, customerId: customer.id });
  return { link: serialiseLink(fresh, customer.name || null), customer, message, waLink: waDeepLink({ msisdn: customer.msisdn, text: message }) };
}

/**
 * STOP / START from a customer's own phone: opt out of (or back into)
 * WaPay-originated requests from EVERY business that holds their number.
 * Returns how many customer rows were touched (0 = not anyone's customer).
 */
export async function setCustomerOptOut({ prisma: prismaClient = prisma, msisdn, optedOut }) {
  const n = normaliseCustomerMsisdn(msisdn);
  if (!n) return 0;
  const r = await prismaClient.businessCustomer.updateMany({ where: { msisdn: n }, data: { optedOutAt: optedOut ? new Date() : null } });
  return Number(r?.count || 0);
}

export const MAX_LOGO_BYTES = 64 * 1024;
/**
 * A business logo is a small raster the browser already resized (256px):
 * PNG / JPEG / WebP as a data URL, at most 64 KB decoded. Stored in the
 * business settings JSON: no file storage to run, no public URL to leak.
 */
export function validateLogoDataUrl(raw) {
  const s = String(raw || '');
  const m = s.match(/^data:image\/(png|jpeg|webp);base64,([A-Za-z0-9+/=]+)$/);
  if (!m) return { ok: false, error: 'BAD_LOGO' };
  const bytes = Math.floor((m[2].length * 3) / 4) - (m[2].endsWith('==') ? 2 : m[2].endsWith('=') ? 1 : 0);
  if (bytes > MAX_LOGO_BYTES) return { ok: false, error: 'LOGO_TOO_BIG' };
  return { ok: true, dataUrl: s };
}

export async function setBusinessLogo({ prisma: prismaClient = prisma, businessId, dataUrl }) {
  return updateBusinessSettings({ prisma: prismaClient, businessId, patch: { logo: dataUrl || null } });
}
 ...
 */
/**
 * A business owner is VETTED when their WaPay account passed KYC (Didit,
 * profile.kyc.status VERIFIED) or their number sits on the pilot invite list
 * (WAPAY_BUSINESS_MSISDNS: the founder vouched for them by hand). Vetting is
 * what makes a WaPay-originated message to a stranger accountable
 * (2026-09-10): the sender is a known person, capped per day, named in the
 * message, and blockable with STOP.
 */
export async function ownerVetted({ prisma: prismaClient = prisma, business }) {
  if (!business?.accountId) return false;
  const account = await prismaClient.account.findUnique({ where: { id: business.accountId } }).catch(() => null);
  if (!account) return false;
  if (account.profile && typeof account.profile === 'object' && account.profile.kyc?.status === 'VERIFIED') return true;
  const n = normSa(account.msisdn || account.waId);
  return !!n && businessSignupAllowlist().includes(n);
}
...
export async function customerEligibleForNudge({ prisma: prismaClient = prisma, businessId, customerId }) {
  ...
  if (customer.optedOutAt) return false; // STOP is final until they say START
  const business = await prismaClient.business.findUnique({ where: { id: businessId } }).catch(() => null);
  if (business && (await ownerVetted({ prisma: prismaClient, business }))) return true;
...
export async function sendLinkViaWaPay({ prisma: prismaClient = prisma, business, customer, code, send }) {
  ...
  const text =
    ...
    `Tap to pay: ${url}\n\n` +
    `No fees for you: pay from your WaPay balance or by card. If you don't recognise this business, ignore this message. Send STOP to block payment requests from WaPay businesses.`;
  ...
  // Remember Meta's message id: the status webhook keys delivery receipts on
  // it (sent → delivered → read, or failed), which the portal shows as ticks.
  const waMessageId = r?.data?.messages?.[0]?.id || r?.messageId || null;
  if (waMessageId) {
    await prismaClient.paymentRequest.updateMany({ where: { id: request.id }, data: { waMessageId, deliveryStatus: 'accepted' } }).catch(() => {});
  }
  log('business_nudge_sent', { businessId: business.id, code: request.id, receipts: !!waMessageId });
