...
export async function deliverRequestPaidNotifications({
  ...
  send = { text: sendWhatsAppText, template: sendWhatsAppTemplate },
}) {
  ...
  try {
    ...
    if (meta.requesterNotifiedAt) {
      ...
      if (sent?.ok) delivered = true;
        ...
        // Paid on day 6 = outside the requester's 24h window; the approved
        // template is the only rail that still lands.
    ...
    if (meta.payerNotifiedAt) {
      ...
      if (sent?.ok) delivered = true;
        ...
        // A payer who never messaged us has no service window — only the
        // approved Utility receipt template reaches them.
