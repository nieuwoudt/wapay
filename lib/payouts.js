...
function providerBodyExcerpt(body) {
  ...
  return text.replace(/\d{6,}/g, (d) => `***${d.slice(-3)}`).slice(0, 1500);
}
...
export async function requestPayout({
  ...
  if (result.settlement === 'PENDING') {
    ...
    return { ok: true, status: 'PENDING', reference, amountCents, feeCents: quote.feeCents, providerRef: result.paymentReference || null, outcome: result.outcome };
  }
  ...
  await record('FAILED', { outcome: result.outcome, providerStatus: result.status, httpStatus: result.httpStatus ?? null, providerBody: providerBodyExcerpt(result.body), finalisedAt: new Date().toISOString() });
 ...
 */
/**
 * The three body parameters an approved UTILITY template must take, in this
 * order, to carry a pay-out outcome outside the 24h window. Set the template
 * name in WAPAY_TEMPLATE_PAYOUT_OUTCOME; leave it unset and the other rails
 * are used. Kept beside payoutOutcomeMessage so the two never drift.
 */
export function payoutOutcomeParams(out) {
  const rands = (c) => 'R' + (Number(c || 0) / 100).toFixed(2).replace(/\.00$/, '');
  return {
    amount: rands(out?.amountCents),
    method: PAYOUT_METHODS[out?.method]?.label || 'your pay-out',
...
export async function reconcilePendingPayouts({ prisma: prismaClient = prisma, ledger = ledgerPost, client = null, olderThanMs = 2 * 60 * 1000, initOlderThanMs = 10 * 60 * 1000, limit = 20, now = Date.now(), deadlineMs = 35 * 1000 } = {}) {
  ...
  const all = [...(await pick('PENDING', olderThanMs)), ...(await pick('INIT', initOlderThanMs))]
    .sort((a, b) => new Date(a.requestTs) - new Date(b.requestTs));
  const rows = all.slice(0, take);
  // How many eligible rows this sweep could NOT reach. The daily floor takes
  // five, so with a growing backlog the rest wait a full day each, and a cap
  // nobody can see reads as "nothing to do" (2026-09-19). `pick` itself takes
  // at most `take` per status, so this is a floor on the backlog, never an
  // overstatement: if it is non-zero there is at least that much waiting.
  const backlog = Math.max(0, all.length - rows.length);
  // Non-enumerable so the return stays exactly the array its callers and
  // their tests expect; the count rides along for whoever wants it.
  const withBacklog = (arr) => Object.defineProperty(arr, 'backlog', { value: backlog, enumerable: false });
  if (rows.length === 0) return withBacklog([]);
  const c = client || new OttPayoutClient({ timeoutMs: 8000 });
  const results = [];
...
export async function sweepPayoutsAndNotify({
  ...
  const counts = { swept: 0, settled: 0, failed: 0, pending: 0, needsOperator: 0, unchecked: 0, errors: 0, notified: 0, notifyFailed: 0, backlog: 0 };
  ...
  // Eligible rows this sweep could not reach. Surfaced so a daily floor that
  // is too small to keep up is visible as a number rather than as silence.
  counts.backlog = Number(results?.backlog) || 0;
  const notified = [];
  let sendFn = send;
  for (const r of results) {
    ...
    // Whenever the ledger has moved and the customer was NOT told, the fact
    // goes into the history the agent reads, so its next turn is not looking
    // at a movement list that changed for no reason it can see. There are
    // three ways to end up here and only one of them was covered
    // (2026-09-18): a failed send, an account we cannot address, and a throw.
    const recordUntold = async () => {
      const { recordMoneyEvent } = await import('./notify.js');
      await recordMoneyEvent({
        prisma: prismaClient,
        accountId: r.accountId,
        text: payoutOutcomeMessage(r),
        kind: 'reconcile',
        refs: { reference: r.reference, status: r.status },
      });
    };
    ...
    if (!account?.waId) {
      ...
      log('payout_notify_failed', { reference: r.reference, reason: 'NO_WA_ID' });
      await recordUntold();
      continue;
    }
    try {
      // The sweep runs at 02:00 and on a cron: by definition the customer may
      // be far outside their 24h window, where Meta ACCEPTS a free-form text
      // and drops it. notifyCustomer picks the rail that actually crosses
      // (C21; before this a pay-out that settled overnight was never told).
      if (!sendFn) {
        const { notifyCustomer } = await import('./notify.js');
        sendFn = ({ to, text }) => notifyCustomer({
          to,
          accountId: r.accountId,
          text,
          kind: 'receipt',
          templateEnv: 'WAPAY_TEMPLATE_PAYOUT_OUTCOME',
          templateParams: [payoutOutcomeParams(r).amount, payoutOutcomeParams(r).method, payoutOutcomeParams(r).reference],
        });
      }
      const sent = await sendFn({ to: account.waId, text: payoutOutcomeMessage(r) });
      // Neither rail throws: a transport failure comes back as ok:false and
      // must not be counted as a customer told.
