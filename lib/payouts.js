...
/** Every pay-out still PENDING after `olderThanMs`, oldest first, each reconciled. */
export async function reconcilePendingPayouts({ prisma: prismaClient = prisma, ledger = ledgerPost, client = null, olderThanMs = 2 * 60 * 1000, limit = 20, now = Date.now() } = {}) {
  const rows = await prismaClient.providerRequest.findMany({
    where: { route: 'ott-payout', status: 'PENDING', requestTs: { lt: new Date(now - olderThanMs) } },
