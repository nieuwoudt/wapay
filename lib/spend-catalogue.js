...
/** The bullet list of live spend destinations, built from data. */
  ...
  if (wicodeLive) {
    ...
    if (partners.length > 0) {
      const parts = partners
        .map((p) => `${p.name} (${p.coverage}, ${p.redeemAt})`)
        .join(' and ');
      lines.push(`⛽ *Fuel vouchers* for participating stations: ${parts}`);
    }
    const shops = catalogue().retail.filter((m) => m.advertised);
    if (shops.length > 0) {
      lines.push(`🛒 *Shopping vouchers* at ${shops.map((m) => m.name).join(', ')}`);
    }
  }
  return lines.join('\n');
}
...
export function redemptionGuide(voucherType) {
  if (voucherType === 'FUEL_WICODE') {
    ...
    return (
      ...
      `1️⃣ Drive to a participating station: ${where}.\n` +
      `2️⃣ BEFORE they start filling up, tell the attendant you are paying with a wiCode and show them the number.\n` +
