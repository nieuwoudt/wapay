 ...
 */
import { feeFacts, payoutLive } from './fee-facts.js';
import { howItWorksFacts } from './how-it-works.js';
import { ottAcceptedFacts as ottAcceptedFactsData } from './ott-acceptance.js';
...
export function cashoutScript() {
  if (process.env.WAPAY_PAYOUT_ENABLED === 'true') {
    return (
      `💸 Withdrawals are live! Say *withdraw R200* (any amount from R20) and choose PayShap to your bank in minutes, a bank transfer, or cash at an Absa ATM or a Pick n Pay / Boxer till. A once-off identity check applies the first time.\n\n` +
      `Your WaPay money also works for:\n` +
 ...
 */
export function spendDestinationLines({ wicodeLive = isWicodeLive(), withdrawLive = payoutLive() } = {}) {
  const lines = [
  ...
  // Withdrawals (2026-09-13): the line exists only while the payout rail is live.
  if (withdrawLive) lines.splice(4, 0, `🏧 *Withdraw* to your bank in minutes, or as cash at an Absa ATM or a Pick n Pay / Boxer till ("withdraw R200")`);
  if (wicodeLive) {
    const retailerLines = [];
...
export function ottAcceptedFacts() {
  // Data-driven since 2026-09-15 (lib/ott-acceptance.js): named non-betting partners, named non-acceptors.
  return ottAcceptedFactsData();
}
 ...
 */
export function buildSpendDestinationsReply({ wicodeLive = isWicodeLive(), withdrawLive = payoutLive() } = {}) {
  ...
  return (
    ...
    spendDestinationLines({ wicodeLive, withdrawLive }) +
    comingSoon +
    `\n\nJust tell me what you would like to do. 😊`
  );
}
 ...
 */
export function buildBrainKnowledge({ wicodeLive = isWicodeLive(), withdrawLive = payoutLive(), capabilityLines = null } = {}) {
  const parts = [
  ...
  // Per-customer registry lines (docs/AGENT_ARCHITECTURE_V2.md C7): only what
  // is live for THIS customer, with the exact start command and fee.
  if (Array.isArray(capabilityLines) && capabilityLines.length) {
    parts.push(`WHAT THIS CUSTOMER CAN DO TODAY (only these; the start commands are exact):\n${capabilityLines.join('\n')}`);
  }
  if (wicodeLive) {
    const partners = advertisedFuelPartners();
    parts.push(
  ...
  // CASH-OUT POSITION follows the SAME switch as the home menu and the
  // withdraw flow (founder screenshots 2026-09-13: with payouts live the AI
  // was still fed "coming soon" from here and told customers exactly that).
  parts.push(
    withdrawLive
      ? `WITHDRAWALS ARE LIVE: the customer types "withdraw" and an amount (R20 to R3000) and chooses PayShap to their own bank account in minutes, a bank transfer to an account number, or cash at an Absa or Nedbank ATM, a Pick n Pay / Boxer till, or as an FNB eWallet collected at any FNB ATM, with no bank account needed (the bank also asks for the customer's ID number). A once-off identity check applies before the first withdrawal. For ANY withdrawal or cash-out ask, tell them to type "withdraw R<amount>" and quote the flat fees from the FEES block. NEVER say withdrawals are coming soon or unavailable.`
  ...
  parts.push(feeFacts({ withdrawLive }));
  parts.push(howItWorksFacts({ wicodeLive, withdrawLive, fuelPartners: advertisedFuelPartners().map((p) => p.name), ottFacts: ottAcceptedFacts() }));
  return parts.join('\n\n');
}
