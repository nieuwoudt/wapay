...
export function cashoutScript() {
  if (process.env.WAPAY_PAYOUT_ENABLED === 'true') {
    return (
      `💸 Withdrawals are live! Say *withdraw R200* (any amount from R20) and choose PayShap to your bank in minutes, a bank transfer, or cash at an Absa or Nedbank ATM. A once-off identity check applies the first time.\n\n` +
...
export function spendDestinationLines({ wicodeLive = isWicodeLive(), withdrawLive = payoutLive() } = {}) {
  ...
  if (withdrawLive) lines.splice(4, 0, `🏧 *Withdraw* to your bank in minutes, or as cash at an Absa or Nedbank ATM ("withdraw R200")`);
  if (wicodeLive) {
    const retailerLines = [];
...
export function buildBrainKnowledge({ wicodeLive = isWicodeLive(), withdrawLive = payoutLive() } = {}) {
  ...
  parts.push(
    withdrawLive
      ? `WITHDRAWALS ARE LIVE: the customer types "withdraw" and an amount (R20 to R3000) and chooses money to their own bank by cellphone number in minutes, a bank transfer to an account number, or cash at an Absa or Nedbank ATM with no bank account needed. A once-off identity check applies before the first withdrawal. For ANY withdrawal or cash-out ask, tell them to type "withdraw R<amount>" and quote the flat fees from the FEES block. NEVER say withdrawals are coming soon or unavailable.`
