...
function extractProductHint(text = '') {
  const t = String(text || '').toLowerCase();
  // ORDERING INVARIANT: explicit product words MUST be checked before the
  // generic send-to rule below. "send R50 airtime to 084..." is a gift of
  // AIRTIME, not SEND_MONEY; only a bare "send R30 to 084..." is a cash send.
