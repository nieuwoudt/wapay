...
/**
 * A fact the customer stated about themself, kept after they confirmed it.
 *
 * This is the one memory the model may ASK for, and it is written here, by
 * the runtime, on an explicit yes, and nowhere else (2026-09-18). It used to
 * be written inside the `propose_note` tool at the moment the model called
 * it, which made the model the author of a customer fact and broke the rule
 * at the top of this file. The tool now only proposes; the processor asks the
 * question and calls this on the answer.
 *
 * Capped at NOTES_MAX, newest last, and a repeat of an existing note moves it
 * to the end rather than storing it twice.
 */
