...
/** Shallow-merge a patch into the profile. Best-effort. */
export async function updateProfile({ prisma: prismaClient = prisma, accountId, patch }) {
  try {
    ...
    const current = await getProfile({ prisma: prismaClient, accountId });
    const next = { ...current, ...patch, updatedAt: new Date().toISOString() };
    await prismaClient.account.update({ where: { id: accountId }, data: { profile: next } });
    return next;
  } catch (error) {
    console.error(JSON.stringify({ type: 'profile_write_error', accountId, error: error?.message }));
    return null;
  }
}
