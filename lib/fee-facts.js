...
function withdrawLines(s) {
  ...
  return `🏧 Withdrawals: ${R(s.withdraw.payshapCents)} to your bank by cellphone number in minutes, ${R(s.withdraw.rtcCents)} for a bank transfer, and cash at an Absa or Nedbank ATM for ${R(cs[0][1])} up to ${R(cs[0][0])}, ${R(cs[1][1])} up to ${R(cs[1][0])}, ${R(cs[2][1])} above that. Say "withdraw R200" to start.`;
}
