...
function withdrawLines(s) {
  ...
  return `🏧 Withdrawals: ${R(s.withdraw.payshapCents)} to your bank account by PayShap in minutes, ${R(s.withdraw.rtcCents)} for a bank transfer, and cash at an Absa ATM or a Pick n Pay / Boxer till for ${R(cs[0][1])} up to ${R(cs[0][0])}, ${R(cs[1][1])} up to ${R(cs[1][0])}, ${R(cs[2][1])} above that. Say "withdraw R200" to start.`;
}
 ...
 */
export function feeAnswer(topic = 'general', amountCents = null) {
  const s = feeSchedule();
  switch (topic) {
