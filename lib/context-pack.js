...
// Raised from 1800 on 2026-09-19 to make room for what the customer has told
// us about themselves. The founder's direction is to remember as much as
// possible from every interaction, and a memory the model never sees is not a
// memory: notes are rendered into this block, so the block had to grow.
export const RECORD_MAX_CHARS = 2600;
/** How many remembered facts reach the prompt, newest last. */
export const NOTES_IN_RECORD = 8;
...
export async function loadContextPack({ prisma, account, now = new Date(), movementLimit = 10, turns = [] }) {
  ...
  // Habits (docs/AGENT_ARCHITECTURE_V2.md C16): computed from the rows
  // already fetched, never remembered by the model.
  const habits = computeHabits({ rows, movements, now });
  ...
  // What the customer has told us about themselves, newest last. Written
  // deterministically by lib/user-profile.js addNote; read here so the agent
  // can actually use it. Until 2026-09-19 notes were stored and never shown
  // to the model, which made remembering them pointless.
  const notes = (Array.isArray(profile.notes) ? profile.notes : [])
    .filter((n) => n && typeof n === 'object' && typeof n.text === 'string' && n.text.trim())
    .map((n) => ({ text: n.text.trim(), at: n.at || null }));

  return {
    habits,
    notes,
    memoryOptOut: profile.memoryOptOut === true,
    accountId,
 ...
 */
/**
 * Habits from the provider rows: the most frequent successful airtime
 * target and amount, the last electricity meter (masked), the deposit rail
 * used most, and how active the account is. Pure; safe on partial input.
 */
export function computeHabits({ rows = [], movements = [], now = new Date() } = {}) {
  const ok = (r) => r && normaliseProviderStatus(r.status) === 'SUCCESS';
  const airtime = rows.filter((r) => ok(r) && /airtime/.test(String(r.route || '')));
  const byTarget = new Map();
  for (const r of airtime) {
    const m = r.metadata || {};
    const key = String(m.msisdn || '');
    if (!key) continue;
    const e = byTarget.get(key) || { count: 0, amounts: new Map(), network: m.vendorName || m.network || null };
    e.count += 1;
    const a = toCents(m.amountCents);
    e.amounts.set(a, (e.amounts.get(a) || 0) + 1);
    byTarget.set(key, e);
  }
  let usualAirtime = null;
  if (byTarget.size) {
    const [msisdn, e] = [...byTarget.entries()].sort((x, y) => y[1].count - x[1].count)[0];
    const amountCents = [...e.amounts.entries()].sort((x, y) => y[1] - x[1])[0][0];
    usualAirtime = { msisdnMasked: maskMsisdn(msisdn), amountCents, network: e.network, count: e.count };
  }
  const elec = rows.filter((r) => ok(r) && (/electricity/.test(String(r.route || '')) || r.metadata?.meterNumber)).sort((a, b) => new Date(b.requestTs) - new Date(a.requestTs))[0];
  const lastMeterTail = elec?.metadata?.meterNumber ? String(elec.metadata.meterNumber).slice(-4) : null;
  const deposits = rows.filter((r) => ok(r) && r.route === 'deposit').length;
  const loads = rows.filter((r) => ok(r) && r.route === 'ott-redeem').length;
  const depositRail = deposits || loads ? (deposits >= loads ? 'card or EFT' : 'voucher at a till') : null;
  const since = new Date(now.getTime() - 30 * 24 * 3600 * 1000);
  const active30d = movements.filter((m) => m.status === 'SUCCESS' && m.at instanceof Date && m.at >= since).length;
  const bits = [];
  if (usualAirtime) bits.push('usual airtime ' + formatRands(usualAirtime.amountCents) + ' for ' + usualAirtime.msisdnMasked + (usualAirtime.network ? ' (' + usualAirtime.network + ')' : ''));
  if (lastMeterTail) bits.push('last meter …' + lastMeterTail);
  if (depositRail) bits.push('usually adds money by ' + depositRail);
  bits.push(active30d + ' completed movement' + (active30d === 1 ? '' : 's') + ' in the last 30 days');
  return { usualAirtime, lastMeterTail, depositRail, active30d, summary: bits.join('; ') };
}
...
export function renderCustomerRecord(pack, { now = new Date() } = {}) {
  ...
  if (p.habits?.summary) tail.push('Habits: ' + p.habits.summary + '.');
  // Things the customer told us. These are THEIR words about themselves, never
  // a number and never a balance: the writer refuses digits, so nothing here
  // can be mistaken for a figure the model may quote.
  const notes = Array.isArray(p.notes) ? p.notes : [];
  if (notes.length) {
    tail.push('What they have told you about themselves:');
    for (const n of notes.slice(-NOTES_IN_RECORD)) tail.push(`- ${n.text}`);
  }
