...
/**
 * Is this msisdn a beneficiary the account already saved? The relationship
 * gate for directed payment requests — only people you've sent to or shared
 * as a contact can be reached in-chat, never an arbitrary number (abuse
 * review 2026-08-25: raw-number targeting was a phishing + enumeration vector).
 *
 * @returns {Promise<boolean>}
 */
export async function isSavedBeneficiary({ prisma: prismaClient = prisma, accountId, msisdn }) {
  const m = String(msisdn || '').trim();
  if (!accountId || !m) return false;
  try {
    const hit = await prismaClient.beneficiary.findFirst({ where: { accountId, msisdn: m } });
    return Boolean(hit);
  } catch (error) {
    console.error(JSON.stringify({ type: 'beneficiary_check_error', accountId, error: error?.message }));
    return false;
  }
}
