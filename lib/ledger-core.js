...
export const FEES = {
  ...
  load: {
    ...
    // Adumo Online (Nedbank acquiring via SHB), 2026-09-10: card, Instant EFT,
    // Capitec Pay, Apple/Google Pay. Same shape as PayFast: face credited, the
    // customer's fee booked as revenue, the processor's cost settled in clearing.
    [RAIL.ADUMO]: { discountBps: 0, creditPolicy: 'FACE' },
  },
   ...
   */
  // Founder 2026-09-11 (payout rail live in test): +R2 on every method over the
  // 2026-08-10 numbers, so each pay-out carries real margin after OTT's cost + VAT:
  // PayShap R8 (cost R2.88 → +R5.12), RTC R10 (cost R5.18 → +R4.82), CashSend
  // R18 / R23 / R30 (cost R11.45 + 0.35% → +R4 to +R8).
  cashout: {
    [RAIL.PAYAT]: { railCostExVatCents: 865, bands: [[Infinity, 1400]] },
    PAYSHAP: { railCostExVatCents: 250, bands: [[Infinity, 800]] },
    RTC: { railCostExVatCents: 450, bands: [[Infinity, 1000]] },
    // Nedbank Cardless Withdrawal (OTT providerCode 4): Annexure A prices it like ABSA CashSend.
    NEDCASH: { railCostExVatCents: 996, switchingBps: 30, bands: [[70000, 1800], [150000, 2300], [Infinity, 3000]] },
    // FNB e-wallet (OTT providerCode 1): NOT in Annexure A. Cost ASSUMED equal to CashSend until OTT
    // confirms the rate (asked 2026-09-15); the customer fee mirrors CashSend so the margin test holds.
    EWALLET: { railCostExVatCents: 996, switchingBps: 30, bands: [[70000, 1800], [150000, 2300], [Infinity, 3000]] },
    CASHSEND: {
  ...
  commissionBps: {
    ...
    LIFESTYLE: 500,
    BETTING: 1000,
    // wiCode fuel via UniFuel/Yoyo: 0 until the inter-company/Yoyo rate is
    // agreed in writing — never book revenue without a signed rate (the
    // voucherGift discipline). Execute passes WAPAY_WICODE_COMMISSION_BPS
    // as commissionBpsOverride once set.
    FUEL: 0,
  },
};
...
export function buildSpend({
  ...
  // Optional customer-facing flat fee ON TOP of the sale (quote of record —
  // whatever the preview quoted is what gets booked). 0 = no fee line.
  flatFeeCents = 0,
}) {
  assertMoney(saleCents, 'saleCents');
  if (saleCents === 0) throw new Error('saleCents must be greater than zero');
  assertMoney(flatFeeCents, 'flatFeeCents');
  ...
  const postings = [
    { accountCode: ACCT.wallet(accountId, balanceType), debitCents: saleCents + flatFeeCents },
  ...
  if (flatFeeCents > 0) {
    postings.push({ accountCode: ACCT.feeRevenue(category), creditCents: flatFeeCents });
  }
  ...
  return entry({
    ...
    postings,
    meta: { accountId, category, saleCents, supplierCents, commissionCents, flatFeeCents, rail, balanceType },
  });
}
