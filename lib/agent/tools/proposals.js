 ...
 * at its FIRST step. Nothing here touches the ledger or a provider, and only
 * `propose_note` touches a database: it keeps what the customer told us about
 * themselves, through the single validated writer in lib/user-profile.js
 * (founder direction 2026-09-19, remember as much as possible).
 *
...
// The ONE database writer any tool touches, and only for the customer's own
// words about themselves. Every other tool here is pure (2026-09-19).
import { addNote } from '../../user-profile.js';
...
/**
 * Remember something the customer said about themselves.
 *
 * FOUNDER DIRECTION 2026-09-19: remember as much as possible from every
 * interaction, unless the customer has told us not to. So this KEEPS the note
 * straight away. It asked for confirmation for one day, and asking was wrong:
 * a question most people never answer meant almost nothing was ever kept.
 *
 * The tool validates and delegates; the write itself lives in
 * `lib/user-profile.js addNote`, which is the one place that touches the
 * profile and the one place that honours the customer's opt-out. The digit
 * ban in `noteRejection` is money safety and stays: no balance, amount, PIN or
 * account number can enter memory through here.
 */
export async function executeProposeNote(args = {}, ctx = {}) {
  try {
    ...
    if (!accountId || !ctx.prisma) return { ok: false, accepted: false, note: null, error: 'NO_ACCOUNT' };
    const wrote = await addNote({ prisma: ctx.prisma, accountId, text, now: ctx.now || null });
    if (wrote?.optedOut) return { ok: true, accepted: false, note: null, reason: 'MEMORY_OFF' };
    if (!wrote) return { ok: false, accepted: false, note: null, error: 'PROFILE_WRITE_FAILED' };
    return { ok: true, accepted: true, note: wrote.note, count: wrote.count };
  } catch (e) {
    return { ok: false, accepted: false, note: null, error: String(e?.message || e).slice(0, 160) };
  }
}
