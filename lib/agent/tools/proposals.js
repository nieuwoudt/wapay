 ...
 * database (the one exception, propose_note, writes a number-free note to
 * the customer's own profile and nothing else).
 *
...
export async function executeProposeNote(args = {}, ctx = {}) {
  try {
    ...
    const at = (ctx.now instanceof Date ? ctx.now : new Date()).toISOString();
    const profile = await getProfile({ prisma: ctx.prisma, accountId });
    const existing = (Array.isArray(profile.notes) ? profile.notes : [])
      .filter((n) => n && typeof n === 'object' && typeof n.text === 'string' && n.text !== text)
      .slice(-(NOTES_MAX - 1));
    const notes = [...existing, { text, at }];
    // updateProfile merges ONE key atomically in Postgres; every other profile key is untouched.
    const merged = await updateProfile({ prisma: ctx.prisma, accountId, patch: { notes } });
    if (!merged) return { ok: false, accepted: false, note: null, error: 'PROFILE_WRITE_FAILED' };
    return { ok: true, accepted: true, note: { text, at }, count: notes.length };
  } catch (e) {
