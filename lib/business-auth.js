...
import { authTemplateComponents } from '@wapay/whatsapp';
...
export async function verifyBusinessOtp({ prisma: prismaClient = prisma, msisdn, code, source }) {
  try {
    ...
    // Every live code is consumed by this ONE attempt, and the attempt
    // succeeds if it matches any of them. Until 2026-09-06 only the newest
    // code was compared, so a code the owner already held from the chat
    // stopped working the moment the portal minted a newer one (which Meta
    // then dropped outside the 24h window): BUGLOG #40. Consuming all of
    // them keeps "one attempt per code": a wrong guess burns every live code.
    const live = await prismaClient.otpCode.findMany({
      where: { accountId: account.id, code: { startsWith: OTP_PREFIX }, consumedAt: null, expiresAt: { gt: new Date() } },
      orderBy: { createdAt: 'desc' },
      take: BUSINESS_OTP_MAX_PER_DAY,
    });
    ...
    for (const row of live) {
      ...
      if (consumed.count !== 1) continue; // raced away by a parallel attempt: never compared here
      consumedCount += 1;
      const expected = Buffer.from(row.code, 'utf8');
      if (expected.length === got.length && crypto.timingSafeEqual(expected, got)) matched = true;
    }
    if (!consumedCount) return { ok: false };
    if (!matched) {
