 ...
 *   PAYSHAP  : to the customer's bank via their cellphone number (ShapID)
...
export function matchWithdrawAsk(text) {
  ...
  if (!/\b(withdraw(?:al|als)?|cash ?out|pay ?out|payshap|cash ?send|cash at|atm|(?:to|into) my bank|my bank account|bank transfer|onttrek|khipha imali)\b/i.test(t)) return null;
...
const methodMenu = (balanceCents) =>
  ...
  `1️⃣ *PayShap* to your bank, using your cellphone number: ${R(quotePayout({ method: 'PAYSHAP', amountCents: MIN_PAYOUT_CENTS }).feeCents)} fee, arrives in minutes\n` +
...
function recipientAsk(data) {
  if (data.method === 'PAYSHAP') return { state: 'PAYOUT_MOBILE', data, text: `Which cellphone number is registered for PayShap at your bank? Reply *mine* to use this WhatsApp number, or type the number.` };
...
function confirmAsk(data) {
  ...
  const where = data.method === 'RTC' ? `account ${r.account_number} at ${r.branch_name || r.branch_code}` : data.method === 'PAYSHAP' ? `your bank via PayShap (${r.mobile})` : `an Absa or Nedbank ATM (collection code to ${r.mobile})`;
  return {
...
export async function handleWithdrawReply({ prisma: prismaClient = prisma, account, state, data = {}, text, deps = {} }) {
  ...
  if (state === 'PAYOUT_BRANCH') {
    ...
    return confirmAsk({ ...data, recipient: { ...data.recipient, ...b } });
  }
  if (state === 'PAYOUT_ID') {
    const digits = t.replace(/\D/g, '');
