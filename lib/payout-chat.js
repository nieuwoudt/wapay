...
const methodLine = (method, limits, balanceCents = null) => {
  ...
  if (method === 'PAYSHAP') return '*PayShap* to your bank account, instant: ' + fee + ' fee, arrives in minutes (from ' + R(min) + ')' + short;
  if (method === 'RTC') return '*Bank transfer* to an account number: ' + fee + ' fee, usually within the hour (from ' + R(min) + ')' + short;
  if (method === 'NEDCASH') return '*Cash at a Nedbank ATM*, code by SMS, no bank account needed: from ' + fee + ' fee (from ' + R(min) + ')' + short;
  if (method === 'EWALLET') return '*FNB eWallet*: cash at any FNB ATM with the code sent by SMS, no bank account needed: from ' + fee + ' fee (from ' + R(min) + ')' + short;
  return '*Cash at an Absa ATM* or a Pick n Pay / Boxer till, no bank account needed: from ' + fee + ' fee (from ' + R(min) + ')' + short;
};
...
function methodUnaffordable(data) {
  ...
  const alts = options.filter((m) => m !== data.method && affordableMaxCents({ method: m, balanceCents: data.balanceCents, limits: data.limits }) != null);
  const altText = alts.length
    ? ' Reply ' + alts.map((m) => '*' + (options.indexOf(m) + 1) + '*').join(' or ') + ' for ' + alts.map((m) => METHOD_SHORT[m]).join(' or ') + ', or add money first.'
    : ' Add money first, or "cancel".';
  return {
    state: 'PAYOUT_METHOD',
    data: { ...data, method: null },
    text: 'With ' + R(data.balanceCents) + ' you cannot use ' + (METHOD_SHORT[data.method] || PAYOUT_METHODS[data.method]?.label) + ' yet: the ' + R(lim.minCents) + ' minimum plus the ' + R(need - lim.minCents) + ' fee is ' + R(need) + '.' + altText,
  };
}
...
function validateAmount(data, amountCents) {
  ...
  if (amountCents < lim.minCents) {
    ...
    const options = data.options || ALL_METHODS;
    const alts = options.filter((m) => m !== data.method && limitsFor({ ...data, method: m }).minCents <= amountCents);
    const altText = alts.length
      ? ` Reply *back* and choose ${alts.map((m) => `*${options.indexOf(m) + 1}* (${METHOD_SHORT[m]}, from ${R(limitsFor({ ...data, method: m }).minCents)})`).join(' or ')}, or type an amount of ${R(lim.minCents)} or more.`
      : ` Please type an amount of ${R(lim.minCents)} or more, or "cancel".`;
    return { state: 'PAYOUT_AMOUNT', data, text: `${R(amountCents)} is below the ${R(lim.minCents)} minimum for ${METHOD_SHORT[data.method] || PAYOUT_METHODS[data.method]?.label}.${altText}` };
  }
  ...
  if (q.totalCents > data.balanceCents) {
    ...
    return { state: 'PAYOUT_AMOUNT', data, text: 'That is more than you have once the ' + R(q.feeCents) + ' fee is added. You can withdraw up to ' + R(cap) + ' by ' + PAYOUT_METHODS[data.method].label + '.' };
  }
  return null;
}
...
export async function handleWithdrawReply({ prisma: prismaClient = prisma, account, state, data = {}, text, deps = {} }) {
  ...
  if (state !== 'PAYOUT_KYC' && state !== 'PAYOUT_METHOD' && /^\W*(menu|back|change|change method|other|options|methods?|start over)\W*$/i.test(t)) {
    return { state: 'PAYOUT_METHOD', data: { ...data, method: null, recipient: {} }, text: methodMenu(data.balanceCents, data.options || ALL_METHODS, data.limits || {}) };
  }
