...
function bestAlternativeFor(data, amountCents) {
  ...
  const priced = options
    ...
    .filter(Boolean)
    .sort((a, b) => a.feeCents - b.feeCents || options.indexOf(a.method) - options.indexOf(b.method));
...
function offerSwitch({ data, amountCents, lead }) {
  ...
  return {
    ...
    text: lead + ' ' + (METHOD_SHORT[alt.method] || PAYOUT_METHODS[alt.method]?.label) +
      ' takes ' + R(amountCents) + ' for a ' + R(alt.feeCents) + ' fee, so ' + R(alt.totalCents) +
      ' leaves your balance. Reply *YES* to switch to that, or type another amount.',
  };
}
...
export async function handleWithdrawReply({ prisma: prismaClient = prisma, account, state, data = {}, text, deps = {} }) {
  ...
  if (state === 'PAYOUT_AMOUNT') {
    ...
    if (data.offerMethod && /^\W*(?:yes|yebo|ja|yeah|yep|ok|okay|sure|please|do it|switch|proceed|continue)\W*$/i.test(t)) {
      return nextAfterMethod({ account, data: { ...data, method: data.offerMethod, amountCents: data.offerAmountCents, offerMethod: null, offerAmountCents: null } });
    }
