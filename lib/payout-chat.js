...
function extraStepClause(data, method) {
  ...
  const adds = needs(method).includes('id_number') && !needs(data.method).includes('id_number') && !data.recipient?.id_number;
...
export async function handleWithdrawReply({ prisma: prismaClient = prisma, account, state, data = {}, text, deps = {} }) {
  ...
  if (state === 'PAYOUT_METHOD') {
    const options = data.options || ALL_METHODS;
    // "YES" takes the method the last reply offered.
  ...
  if (state === 'PAYOUT_AMOUNT') {
    ...
    if ((data.offerMethod || data.offerAmountCents) && /^\W*(?:yes|yebo|ja|yeah|yep|ok|okay|sure|please|do it|switch|proceed|continue)\W*$/i.test(t)) {
...
export async function executeWithdraw({ prisma: prismaClient = prisma, account, data, deps = {} }) {
  ...
  if (out.ok && out.status === 'PENDING') {
    ...
    if (!payoutHandedOver(out.outcome)) {
      return { state: null, done: true, text: `⏳ I could not get confirmation from the bank rail just now, so I am checking on it (reference ${out.reference}). Your ${R(out.amountCents)}${Number.isInteger(out.feeCents) ? ` plus the ${R(out.feeCents)} fee` : ''} is held, not spent, and I will message you here either way.` };
    }
    return { state: null, done: true, text: `⏳ *In progress.* ${R(out.amountCents)} has been handed to the bank rail (reference ${out.reference}). I'll message you here the moment the bank confirms it, and if it does not go through the amount and the fee come straight back to your balance.` };
  }
