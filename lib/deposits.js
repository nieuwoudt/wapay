 ...
 */
export const PAYREQ_FREE_BELOW_CENTS = Number(process.env.WAPAY_PAYREQ_FREE_BELOW_CENTS ?? 5000);

/**
 * Per-rail card economics (2026-09-10). PayFast: R2.30 + 4.20% (its true
 * cost is R2.30 + 3.68% credit / 4.02% debit incl. VAT). Adumo (Nedbank via
 * SHB): R1 + 2.5% by default (founder 2026-09-10: "a little more competitive
 * than iKhokha, Yoco and PayFast"), against a true cost of R0.92 + 1.67%
 * debit / 2.82% credit incl. VAT: about 0.45% blended margin at a 60/40
 * debit/credit mix, a bounded few-rand loss on a pure credit-card ticket,
 * and the lowest headline in the market (iKhokha 2.85%, Yoco ≈2.8%, PayFast
 * 3.2% + R2, all ex VAT). This is the floor for a single blended fee; below
 * it credit cards are processed at a loss. Both rails are env-tunable.
 */
/**
 * VAT on OUR fee (founder 2026-09-10: "we will be charging VAT"). WaPay is not
 * VAT-registered today, so the receiver fee carries none. The day it
 * registers, WAPAY_VAT_REGISTERED=true grosses every card fee up by 15% and
 * the quote says so; nothing else changes. The input VAT on Adumo/PayFast
 * fees then becomes reclaimable, which is what funds a lower headline.
 */
export const VAT_RATE_BPS = 1500;
export function vatRegistered() { return process.env.WAPAY_VAT_REGISTERED === 'true'; }

export function cardRailFee(rail = 'PAYFAST') {
  if (rail === 'ADUMO') {
    return { bps: Number(process.env.WAPAY_ADUMO_FEE_BPS ?? 250), fixedCents: Number(process.env.WAPAY_ADUMO_FEE_FIXED_CENTS ?? 100) };
  }
  return { bps: Number(process.env.WAPAY_DEPOSIT_FEE_BPS ?? 420), fixedCents: Number(process.env.WAPAY_DEPOSIT_FEE_FIXED_CENTS ?? 230) };
}

export function paymentRequestFeeCents(amountCents, rail = 'PAYFAST') {
  if (!Number.isInteger(amountCents) || amountCents <= 0) {
    throw new Error(`paymentRequestFeeCents requires positive integer cents, got ${amountCents}`);
  }
  // SMALL AMOUNTS ARE FREE — a deliberate, bounded subsidy (founder call
  // 2026-08-27). PayFast's fixed R2.30 floor means ANY margin-positive fee on
  // a R20 request is >15% of it, and no wording rescues that; a flat fee is
  // actually WORSE at the bottom than the percentage. So we absorb it below
  // the threshold and say "small amounts are free", which is both true and
  // unanswerable in marketing.
  //
  // The economics: each absorbed payment costs us the PayFast fee (~R2.50–
  // R4.10) and buys a card payer whose number we capture — i.e. a lead at
  // roughly a tenth of the R35–R60 CAC of a Meta ad. This is acquisition
  // spend booked as a fee waiver, not a pricing mistake. Bound it with
  // WAPAY_PAYREQ_FREE_BELOW_CENTS; set to 0 to switch the subsidy off.
  if (amountCents < PAYREQ_FREE_BELOW_CENTS) return 0;

  const { bps, fixedCents } = cardRailFee(rail);
  const exVatCents = Math.ceil((amountCents * bps) / 10000) + fixedCents;
  const rawCents = vatRegistered() ? Math.ceil((exVatCents * (10000 + VAT_RATE_BPS)) / 10000) : exVatCents;
  const fee = Math.ceil(rawCents / 10) * 10;

  // TAPER ACROSS THE THRESHOLD. Without this the schedule is
  // non-monotonic: asking R49 nets R49 (free) while asking R50 nets
  // R45.60 — i.e. asking for MORE pays you LESS, which users find fast and
  // rightly call a bug. Capping the fee at (amount - lastFreeAmount) makes
  // the NET strictly non-decreasing: net = max(amount - fee, threshold-ish).
  // It only bites in the few rand just above the threshold, and extends the
  // same bounded subsidy that already applies below it.
  const maxFeeForMonotonicNet = Math.max(0, amountCents - (PAYREQ_FREE_BELOW_CENTS - 100));
  return Math.min(fee, maxFeeForMonotonicNet);
}

/**
 * The request amount to ASK FOR so the requester NETS `wantCents` after the
 * card fee. Used at compose time so "please pay me R20" can offer "ask R24
 * and receive the full R20" — the requester picks the number on the link and
 * the PAYER always pays exactly what is displayed.
 *
 * ⚠️ This is NOT a surcharge and must never become one: charging the payer
 * more BECAUSE they chose card is prohibited in SA (PayFast T&Cs cl. 5.3,
 * SARB/PASA, Visa/Mastercard — see the no-card-surcharging note). Here the
 * requester chooses a single displayed price that every payer pays.
 *
 * @param {number} wantCents - what the requester wants to receive
 * @returns {number} the amount to display on the link (>= wantCents)
 */
export function grossedUpRequestCents(wantCents) {
  if (!Number.isInteger(wantCents) || wantCents <= 0) {
    throw new Error(`grossedUpRequestCents requires positive integer cents, got ${wantCents}`);
  }
  // Round the suggestion to a WHOLE RAND: "make it R55" is what a person
  // actually says and types, and whole rands parse reliably in chat.
  let ask = Math.ceil(wantCents / 100) * 100;
  for (let i = 0; i < 64; i += 1) {
    if (ask - paymentRequestFeeCents(ask) >= wantCents) return ask;
    ask += 100;
  }
  return ask;
}
...
const DEPOSIT_STATUS_PATTERNS = [
  // "did/has/is/was … payment|deposit|withdrawal|money … go through/arrive/clear/…"
  // Withdrawals were added 2026-09-16: "did my withdrawal go through" must
  // reach the same ledger lookup (which now covers pay-outs), not the AI.
  /\b(?:did|has|have|is|was|when\s+(?:will|does|is))\b.{0,30}\b(?:payments?|paymnets?|deposits?|depsits?|deposites?|withdraw\w*|pay-?outs?|cash-?outs?|money|cash)\b.{0,40}\b(?:go(?:ne)?\s+through|went\s+through|arriv\w*|clear\w*|land\w*|receiv\w*|reflect\w*|show\w*|success\w*|credited|paid|done|work\w*|in\s+yet)\b/i,
  // "where is my money / where's my deposit / wheres my payment / where is my withdrawal"
  /\bwh?ere(?:'?s|\s+is|\s+are)\s+(?:my|the)\s+(?:money|payments?|deposits?|withdrawals?|pay-?outs?|cash|funds?)\b/i,
  // "payment status" / "deposit status" / "withdrawal status" / "status of my payment/deposit/withdrawal"
  /\b(?:payments?|deposits?|depsits?|deposites?|withdrawals?|pay-?outs?)\s+status\b|\bstatus\s+of\s+my\s+(?:payments?|deposits?|withdrawals?|pay-?outs?)\b/i,
];
