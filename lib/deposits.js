 ...
 * SHB): R1 + 2.8% by default, against a true cost of R0.92 + 1.67% debit /
 * 2.82% credit incl. VAT: about 0.7% blended margin, level with iKhokha
 * (2.85% ex VAT) and Yoco on small tickets and cheaper above ~R200. Both are
 * env-tunable; the founder sets the final Adumo numbers before enabling it.
 */
export function cardRailFee(rail = 'PAYFAST') {
  if (rail === 'ADUMO') {
    return { bps: Number(process.env.WAPAY_ADUMO_FEE_BPS ?? 280), fixedCents: Number(process.env.WAPAY_ADUMO_FEE_FIXED_CENTS ?? 100) };
  }
