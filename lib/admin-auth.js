...
export async function requestAdminOtp({ prisma: prismaClient = prisma, msisdn, send, sendTemplate }) {
  try {
    ...
    const templateName = process.env.WAPAY_TEMPLATE_ADMIN_OTP || 'otp_register_step_2';
    const diag = { to: maskTail(account.waId), templateName };
    let sent = null;
    if (typeof sendTemplate === 'function') {
      ...
      diag.templateError = sent?.ok ? undefined : String(sent?.error || 'unknown').slice(0, 200);
      diag.templateMessageId = sent?.data?.messages?.[0]?.id;
      diag.templateAcceptedFor = sent?.data?.contacts?.[0]?.wa_id;
    }
