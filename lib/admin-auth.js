...
export async function verifyAdminOtp({ prisma: prismaClient = prisma, msisdn, code }) {
  try {
    ...
    // Every live admin code is consumed by this ONE attempt, and the attempt
    // succeeds if it matches any of them (BUGLOG #40, 2026-09-06: with the
    // newest-only compare, a code already in the admin's chat kept failing
    // whenever the console minted a newer one). One attempt per code still
    // holds: a wrong guess burns every live code.
