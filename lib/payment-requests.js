...
export async function createPaymentRequest({ prisma: prismaClient = prisma, accountId, amountCents, note }) {
  ...
  const [openCount, dayCount] = await Promise.all([
    MAX_OPEN_REQUESTS > 0
      ? prismaClient.paymentRequest.count({
          where: { accountId, status: 'PENDING', expiresAt: { gt: now } },
        })
      : 0,
    MAX_REQUESTS_PER_DAY > 0
      ? prismaClient.paymentRequest.count({
          where: { accountId, createdAt: { gt: new Date(now.getTime() - 24 * 60 * 60 * 1000) } },
        })
      : 0,
  ]);
  if (MAX_OPEN_REQUESTS > 0 && openCount >= MAX_OPEN_REQUESTS) {
  ...
  if (MAX_REQUESTS_PER_DAY > 0 && dayCount >= MAX_REQUESTS_PER_DAY) {
