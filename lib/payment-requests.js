 ...
 *   * by card/EFT via PayFast (the payer pays exactly the face amount; the
 *     banded card fee is deducted from what the REQUESTER receives — founder
 *     decision 2026-08-22 — via the standard ITN machinery)
 *
...
export async function createPaymentRequest({ prisma: prismaClient = prisma, accountId, amountCents, note, business = null }) {
  ...
  const extra = businessId
    ? {
        ...
        // Json? columns take a value or nothing — never a JS null (Prisma 5).
        ...(Array.isArray(business.items) && business.items.length ? { items: business.items } : {}),
