...
export function paymentRequestUrl(code) {
  const shortBase = String(process.env.PAYLINK_BASE_URL || '').replace(/\/+$/, '');
  if (shortBase) return `${shortBase}/${code}`;
  const base = String(process.env.APP_BASE_URL || 'https://wapay.co.za').replace(/\/+$/, '');
