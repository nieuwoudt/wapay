...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      // The customer now has the bundle: a crash from here on must NOT
    ...
    if (holdIdemKey && !providerDelivered) {
      try {
        ...
        logStructured('vas_data_crash_release_failed', {
          previewId,
          idemKey: holdIdemKey,
      ...
      logStructured('vas_data_settle_failed_after_delivery', {
        previewId,
        idemKey: holdIdemKey,
        error: error?.message,
      });
