...
## 1b. Founder review 2 (2026-09-15): the rigid turns, and what changed

| Reply on the phone | Why | Fixed how |
|---|---|---|
| "Are OTT vouchers accepted currently?" answered with categories and "check ottvoucher.com"; "Is it accepted at Checkers?" could not be confirmed | `ottAcceptedFacts()` deliberately named no merchants (most OTT partners are betting operators) and no acceptance data existed | `lib/ott-acceptance.js`: researched partners and non-acceptors by name, 12-digit PIN, single-use, change to an OTT Wallet only at some partners, 36-month expiry, no cash-out. `lookupOttMerchant` gives a yes or no by name; betting names never appear (BUGLOG #49) |
| "Can I buy electricity?" opened the electricity flow ("How much electricity would you like to buy?") | `productQueryIndicators` routes "can I buy…" to the product flow; nothing separates a question from a command | `lib/how-it-works.js`: a question about a capability (no amount, no number) gets the steps, limits, fees and the exact words to start; the hook runs after the fee hook and before the withdraw command matcher (#48) |
| "Can they withdraw the OTT voucher for money?" started a withdrawal ("No problem, switching over… Withdraw from WaPay") | the withdraw matcher fired on the word "withdraw" in any sentence | question-shaped withdraw asks go to the knowledge base; an OTT question wins when the sentence names the voucher (#48) |
| Inside the flow, "If I send someone money, can they withdraw it?" got "Reply 1 for PayShap, 2 for a bank transfer or 3…"; "Does it work when I send cash to an ATM? How do I withdraw the money?" was parsed as method 3 | `PAYOUT_*` states read every reply as step input; "ATM" matched the method by name | `stepInputParses` + `answerAside` + `reprompt`: a question is answered (knowledge base or fee facts) and the step repeated; a name match inside a sentence is not a choice (#48) |
| Founder review 3 (evening): "Hello" answered with "Just the amount" twelve hours after a withdrawal was parked | states never expired; greetings inside a flow were flow input | `stateSetAt` stamp + 30-minute idle expiry; hi / hello / home / menu go home from any flow (#50) |
| "Can I withdraw money?" and "Can I withdraw at an Absa ATM?" got the same four-step wall | one detail answer per topic | question-aware `brief` per topic + offer; YES starts the flow via `HOWTO_OFFER`; "how do I" gets the walkthrough with the bank's collection steps (#51) |
| "20" against Absa's R50 minimum: the same line three times | the amount validator knew only its own limits | names the minimum, the method and the methods that allow the amount; "back" changes method (#52) |
| The founder's ask: "create the steps and knowledge base around each transaction type" | the steps lived only in the flows' prompts, one screen at a time | `TOPICS` in `lib/how-it-works.js` is that knowledge base; `docs/CONVERSATION_KNOWLEDGE_BASE.md` is generated from it, and the AI receives the same HOW IT WORKS block each turn |

This is Phase 0.5 of the plan in section 5: the knowledge base and the acceptance data are the first two entries of the capability registry, and the question gate is the first step away from "every sentence is a command".
