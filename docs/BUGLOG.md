...
- **Root cause:** two. (1) `cleanRecipient` returns every recipient field and fills the absent optionals with `""`, and `performPayout` sent that object as-is; OTT's payout API is a .NET service whose `bank_id` is an `Int32`, so `"bank_id": ""` fails model binding and the request is refused before any payout is created ("The purchase field is required" is the binder reporting the whole body as unreadable). (2) ASP.NET's RFC 7807 problem document carries `status: 400`, and `classifyPayoutStatus(400)` read it as an unknown OTT payout status → PENDING with the customer's money held, instead of a rejection with nothing paid.
- **Fix:** `wireRecipient` drops empty optionals from the wire body and sends integer-typed fields (`bank_id`) as numbers; the hash is computed exactly as before, over the string values with absent optionals as `""` (spec). `isProblemDetails` recognises a problem document (HTTP ≥ 400 with `errors`/`title`/`type` and no `paymentReference`); a 400 that names fields is `REQUEST_INVALID` → RELEASE (nothing paid, the customer is whole immediately), any other problem document is `HTTP_<code>` → PENDING + reconcile. FAILED rows now keep `httpStatus` and a masked 1,500-character body. The customer copy says the fault is ours. `POST /api/internal/payout-probe` (internal key, sandbox host only, no ledger) sends one request exactly as the withdraw flow does and returns the masked wire request and raw response, so "share your request and response" is answered with facts.
- **Guard:** `tests/payout-wire.test.mjs` (wire body has no empty fields and the hash is unchanged; the recorded 17 Sep problem document maps to REQUEST_INVALID/RELEASE; other problem documents keep the hold; the FAILED row carries the HTTP status and a masked body; the customer copy; the probe route is gated, sandbox-only, ledger-free, single-shot and masked).
- **Second finding (same day, sandbox probe):** with the body fixed OTT processed the request and answered status 2 "Invalid Hash", echoing the request as parsed (`amount: 20`, `providerCode: 4`, `bank_id: 0`). OTT hashes its own typed values: an absent `bank_id` is its Int32 default and hashes as `"0"`, not `""`; the amount hashes as the documented 2dp string. Four renderings were tried through the probe; `{ amount: '2dp', bankId: 'zero' }` answered **status 100 "Payment successful", paymentReference 126141** (Nedbank cardless R20, sandbox, 2026-09-23 17:58 SAST, through the probe route, which writes no ledger and no row: the wire format is proven, a customer withdrawal is not yet). That style is now `DEFAULT_HASH_STYLE`. PayShap probes with the same format were created at OTT (126142, 126143) and ended status 97 after more than 20 s; Nedbank (126144) 100 again: the format is right for both providers, the sandbox's PayShap provider is what fails.
- **Lesson:** a supplier's status table is for the supplier's own responses; the framework in front of it has its own vocabulary. Persist the raw response on every failure from day one (the 15 Sep row taught nothing; the 17 Sep row solved it). And when a .NET API echoes its parsed request, read the echo: every default it filled in is a value it hashed.
...
- **Root cause:** two gaps in the provenance guard, both on the "known figure" side. (1) `knownAmountsFromPack` walks the CONTEXT PACK only. The design has always said a figure may come from the record **or a tool result this turn**, and the tool half was never built, so a total or a row that `get_transactions` returned and the pack did not carry read as invented. (2) Zero was never a known figure: `add()` requires `c > 0`, so an honest summary ending "completed spending total: R0" contained a figure in no set and the whole reply was blocked.
- **Fix:** `runAgentTurn` now returns `toolAmountsCents`, every integer `*Cents` field any tool returned this turn, numbers only so no tool text travels with them; `handleAgentTurn` merges them into the QUOTABLE set, never the settled set, so the agent may list and total them while a success claim still has to name money the ledger says settled. `looksLikeReceipt` treats R0 as always known, because "you received R0" is not a claim anyone can be defrauded by.
- **Guard:** `tests/live-failures-2026-09-19.test.mjs` replays both shapes and asserts the anti-fraud property is untouched: a tool-provided figure may be quoted but never called paid, and an invented success claim is still blocked. The chat harness assertion that had been passing only because the guard suppressed a TRUE statement is rewritten: the QA run really does settle an R30 pay-out, so naming it is correct and the scenario now checks that no figure the wallet never saw is called paid.

## 76. "Buy R10 airtime" could not be completed: the phone number was read as a rand amount, and "mine" cancelled the purchase

- **Symptom:** founder live test 2026-09-19, three attempts in a row. "buy R10 airtime" → "Which phone number should I send the airtime to?" → `0787051175` → "❌ Amount must be between R5 and R1000. Please try again later." Answering "mine" instead gave "I've cancelled the airtime purchase." His words: "it's absolutely embarrassing. This used to work."
- **Root cause, two faults in one state.** (1) `parseSlots('0787051175')` returns `amountCents: 78705117500` as well as the msisdn, because a bare run of digits is genuinely ambiguous. `AIRTIME_MSISDN` guarded that by comparing the parsed amount's digit string to a prefix of the msisdn's digit string, which stops matching the moment the number is typed in the normal 0-form: `"78705117500"` is eleven characters and the msisdn is ten, so the guard never fired and R787,051,175.00 went to the preview, which rejected it. The guard was written against a parse that has since changed shape. (2) `"mine"` carries no digits, and the branch that reads "fewer than eight digits" as "the customer wants out" sat ABOVE the self matcher, so it cancelled. A wider self matcher already existed twenty lines below and was unreachable.
- **Fix:** the chosen amount always wins inside the state (`data?.amountCents || filledSlots.amountCents`), which needs no heuristic at all: the question asked was "which number", so the reply is a number. One generous `matchSelfNumber` covers me, mine, myself, my number, my phone, my own, this one, same, self and for me, runs before any cancel branch, and refuses "my mother". The data flow uses the same matcher.
- **Guard:** `tests/live-failures-2026-09-19.test.mjs` pins the real parse (`0787051175` → R787,051,175.00, so the ambiguity is documented rather than assumed away), the stored-amount precedence, the branch ORDER, and the ten phrasings that mean self against six that must not.
