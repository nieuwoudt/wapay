# WaPay chat QA report · 2026-09-06
...
[button:View my payment page -> https://pleasepayme.co.za/PRSXUTKT]
···
🙏 *Pay WaPay QA Harness now* · R250 on WaPay

Tap the link to pay. Free from a WaPay balance, or pay by card:
https://pleasepayme.co.za/PRSXUTKT
```
...
```
Nice to meet you, Thabo 😊 Spaza shops like yours can use WaPay to add value for customers with airtime, data, electricity, send money and payment links.
```
...
```
Got it, Thabo 😊 Your favourite colour is green.
```
...
```
You can use your WaPay money for airtime and data, prepaid electricity, sending money, getting paid with a payment link, and WaPay vouchers for many online stores 💰📱

If you want, I can also show you where OTT vouchers are accepted.
```
...
```
Withdrawals are not available yet, but they’re coming soon 💡
For now, your WaPay money can be spent on airtime, data, prepaid electricity, sending to others, or WaPay vouchers online.
```
...
```
OTT vouchers are accepted at many South African online stores and platforms, including mobile top-up sites, streaming subscriptions, and selected retail and tech stores 💡
For the full up-to-date list, check ottvoucher.com and pick OTT at checkout.
```
...
```
Here’s the simple version: sending money has a flat R3 fee. Requesting money is free for the payer, but if they pay by card a small card fee is taken from what you receive. Deposits by card or EFT use PayFast and may have standard payment fees from the payment method. 💡
```
...
```
🔐 *WaPay for Business code: 704678*
