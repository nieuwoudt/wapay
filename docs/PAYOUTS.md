...
## Status 2026-09-23 (why OTT had no record of our pay-outs; production approved)

OTT (Yaku) answered the sandbox email: the payout status code in the document is the JSON body's `status`, not the HTTP status, and they could find no transaction on the account. Our own record of the founder's 17 September retry explained why: the request never got past OTT's model binder. `cleanRecipient` filled every absent optional with `""`, OTT's `bank_id` is an integer, and the API answered **HTTP 400** with an RFC 7807 problem document (`$.recipient.bank_id: The JSON value could not be converted to System.Int32`; `purchase: The purchase field is required`). Our client then read the document's `status: 400` as an OTT payout status and parked the money (BUGLOG #80).

Fixed the same day: empty optionals are omitted on the wire and `bank_id` goes as a number (hash unchanged); problem documents are recognised (a 400 naming fields releases the hold immediately, anything else keeps it for the sweep); FAILED rows keep the HTTP status and a masked body; the customer is told the fault is ours. `POST /api/internal/payout-probe` (internal key, sandbox host only, no ledger, R50 cap) sends one request exactly as the withdraw flow does and returns the masked wire request and the raw response: the tool for answering "share your request and response".

**Proven on the sandbox the same evening:** after the body fix OTT answered status 2 "Invalid Hash" and echoed its parsed request (`amount: 20`, `bank_id: 0`); the probe tried four hash renderings and `{ amount: '2dp', bankId: 'zero' }` (absent `bank_id` hashed as "0") returned **status 100 "Payment successful", paymentReference 126141** for a Nedbank cardless R20. That is the client's default now (`DEFAULT_HASH_STYLE`). This proves the wire format through the probe route, which writes nothing: no customer withdrawal has completed yet (the three `ott-payout` rows in production are all FAILED, newest 17 September). The next informative test is one R50 withdrawal through the chat. PayShap on the sandbox: two probes got no response within the 20 s client timeout; `GET /api/internal/payout-probe?reference=WP…` (GetPaymentStatus) then showed both WERE created (paymentReference 126142, 126143) and ended **status 97, failed at provider**, while a second Nedbank probe (126144) is 100. So the format is right for both providers; PayShap on the test environment is slow and then rejected on OTT's side (asked in the email). In the chat a PerformPayout timeout is returned as TRANSPORT_INDETERMINATE → the row parks PENDING, the customer hears *Sent*, and the webhook / on-inbound reconcile / sweep finalises it (here: release, money back).

**Production:** OTT approved production pay-outs and issued production credentials (username `WAPAYPOL`; key and password held by the founder, pasted in chat on 2026-09-23 and therefore to be regenerated in the portal once applied). FNB e-wallet stays mapped but is not yet available on OTT's side ("it will eventually come online"); the live provider list decides what the chat offers. Before `OTT_PAYOUT_BASE_URL` moves to `https://payoutapi.ott-mobile.com`: this fix deployed and proven on the sandbox probe; providers activated on the PRODUCTION merchant (portal > Administration > Payout Providers > Add All) or `mapped: []` refuses every request; the webhook URL set on the production merchant; the production float funded (GetBalance shows it); `WAPAY_PAYOUT_ALLOWLIST` kept. Env changes take effect on redeploy only.
