...
A WaPay-originated push ("Also send from WaPay") exists but is **off by default** (`WAPAY_BUSINESS_NOTIFY=true` to enable) and, even then, is allowed only for a customer who has **already paid this business** through a WaPay link (a money-backed relationship, per memory `cross-user-actions-need-consent-gate`). It is informational-only (names the sender as "a WaPay business", plants no state on the recipient, tells them to ignore it if unrecognised), one send per link, capped at `WAPAY_BUSINESS_NUDGES_PER_DAY` (default 20) per business, and rides the same text → Direct Send → template chain as every other proactive notify (`WAPAY_TEMPLATE_BUSINESS_REQUEST` for the template). **Run the read-only adversarial review before flipping the flag.**
...
- OTP over WhatsApp: hashed `biz:` rows in `otp_codes` (never collide with admin `adm:` or customer codes), 10-min expiry, one attempt per code, 1 send/min, 20/day, lockout after 5 burns. Delivery: authentication template candidates (`WAPAY_TEMPLATE_BUSINESS_OTP`, then the admin/onboarding candidates) → free-form fallback; or **in-session**: the owner types `business login` (or `business code` / `portal login`) to WaPay from their phone and the code comes straight back (the BUGLOG #33 inversion). Non-owners get silence.
- Password (optional, set at registration or in Settings): self-contained argon2id in `businesses.passwordHash`, 10+ chars, 5 fails = 15-min lockout, wrong-number and wrong-password answer identically.
- Registration: passing the OTP for a wallet without a business yields a 15-minute registration token; the name is validated (`validateBusinessName`: sanitised, 2–60 chars, may not contain WaPay / PayFast / WhatsApp / Meta / SARS / Eskom / bank names). `WAPAY_BUSINESS_MSISDNS` (optional, comma-separated 27-form) restricts who may register during the pilot; existing businesses always keep signing in.
...
2. Set `WAPAY_BUSINESS_SESSION_SECRET` (`openssl rand -hex 32`), or rely on the existing `WAPAY_ADMIN_SESSION_SECRET`.
3. Optional pilot gate: `WAPAY_BUSINESS_MSISDNS=27xxxxxxxxx` (the laundry owner's WaPay number).
4. Optional host: attach `business.wapay.co.za` to the Vercel project, set `WAPAY_BUSINESS_HOST=business.wapay.co.za` (and exempt it from Vercel's attack-challenge mode, the BUGLOG #34 lesson).
5. Leave `WAPAY_BUSINESS_NOTIFY` unset until the adversarial review of the nudge path has run.
6. **Redeploy** (env changes do nothing until then).

First login for the laundry: they must already have a WaPay wallet (say hi to WaPay on WhatsApp). Then at `/business`: number → code (or `business login` from the phone) → name the business → set a password for the shop computer.
