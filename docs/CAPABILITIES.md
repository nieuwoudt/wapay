...
| Money out (payout rail) | **ON in production (2026-09-11), blocked on the OTT sandbox** | 2026-09-13: `WAPAY_PAYOUT_ENABLED=true`, OTT_PAYOUT_* set, `WAPAY_PAYOUT_KYC=off` for the sandbox window; OTT TEST account authenticates but has R0 float and no active providers, so every request is refused `NO_PROVIDER` before any ledger movement. Probe: `GET /api/internal/payout-status`. Previously: `lib/ott-payout.js` — all 9 OTT Payout endpoints + webhook verification, crypto proven against OTT's published golden vectors, money-safe status→settlement map (never releases a hold on PENDING). Blocked on OTT test credentials + sandbox verification, and **counsel clearance on cash-out** before any customer sees it. |
...
| Conversational spend knowledge | **Live** | 2026-09-13: the knowledge block now follows `WAPAY_PAYOUT_ENABLED` (BUGLOG #43) and carries a FEES block computed from the charging functions (`lib/fee-facts.js`, BUGLOG #44); price questions are answered deterministically before the keyword router. Earlier: Every AI turn carries the data-driven, claim-gated spend catalogue (`lib/spend-catalogue.js`): "where can I spend my money" gets a warm, specific answer (never the bare menu — BUGLOG #34a); cash-out asks get the honest coming-soon script (no dates, partner unnamed); fuel/retail presented as coming soon until `WAPAY_WICODE_LIVE`. |
...
| WaPay for Business (portal + payment-link CRM) | **Built, env-gated (2026-09-04)** | `/business`: a business is a WaPay account wearing a hat — owner signs in with the wallet's WhatsApp number (OTP via template or in-chat `business login`, optional argon2id password), names the business, keeps customers (add / paste-import CSV+vCard / auto-captured payers), sends POS-style itemised "please pay me" links from the portal via the owner's OWN WhatsApp (wa.me prefill; WaPay-originated push is off by default and relationship-gated), and sees per-customer + per-business revenue (12-month series, 3/6/12 totals, outstanding, method split, fees/net) with CSV export. Money rides the existing payment-request rail unchanged. Registration is CLOSED by default: `WAPAY_BUSINESS_MSISDNS` invites numbers, `WAPAY_BUSINESS_SIGNUPS=open` opens it. Sign-up also happens IN the chat (2026-09-06): "for you, or for a business? 1/2" right after onboarding, or `business account` for an existing wallet, then the trading name; `business login` gives the portal code. Onboarding OTP flag `WAPAY_ONBOARDING_OTP=off` (default on) documented in `docs/ONBOARDING.md`. 2026-09-10: functions moved next to the DB (0.3 s dashboards), customers added from the composer, CSV/vCard file drop, QR per link, business logo, WaPay-originated sends for vetted owners with STOP/START and delivery ticks, and a Payouts tab on OTT's rail (`lib/payouts.js`, flag-gated, awaiting the sandbox). Same day: Adumo Online (Nedbank/SHB) hosted-page rail next to PayFast, sandbox-proven, rail-aware receiver fees, off until `WAPAY_ADUMO_ENABLED` (`docs/ADUMO.md`). 2026-09-11: withdrawals in chat ("withdraw R200": PayShap / bank transfer / ATM cash, KYC + PIN) behind `WAPAY_PAYOUT_ENABLED` (`docs/PAYOUTS.md`). Migration `20260904_business` applied to prod 2026-09-04; secret falls back to the admin one; host `WAPAY_BUSINESS_HOST=business.wapay.co.za` once the domain is attached. See `docs/BUSINESS_PORTAL.md` + `docs/BUSINESS_PORTAL_TEST_GUIDE.md`. |
