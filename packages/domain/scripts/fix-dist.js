...
const nestedSrcPath = path.join(distPath, 'packages/domain/src');
...
if (fs.existsSync(nestedSrcPath)) {
  ...
  try {
    fs.rmdirSync(path.join(distPath, 'packages/domain/src'), { recursive: true });
    fs.rmdirSync(path.join(distPath, 'packages/domain'), { recursive: true });
    fs.rmdirSync(path.join(distPath, 'packages'), { recursive: true });
  } catch (e) {
