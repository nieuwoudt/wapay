...
const nestedSrcPath = path.join(distPath, 'providers/ott/src');
...
if (fs.existsSync(nestedSrcPath)) {
  ...
  try {
    fs.rmdirSync(path.join(distPath, 'providers/ott/src'), { recursive: true });
    fs.rmdirSync(path.join(distPath, 'providers/ott'), { recursive: true });
