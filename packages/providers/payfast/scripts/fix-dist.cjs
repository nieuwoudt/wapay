...
const nestedSrcPath = path.join(distPath, 'providers/payfast/src');
...
if (fs.existsSync(nestedSrcPath)) {
  ...
  try {
    fs.rmdirSync(path.join(distPath, 'providers/payfast/src'), { recursive: true });
    fs.rmdirSync(path.join(distPath, 'providers/payfast'), { recursive: true });
